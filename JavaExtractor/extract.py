        ...
        try:
            with multiprocessing.Pool(4) as p:
                result = p.starmap_async(
                    ParallelExtractDir,
                    zip(itertools.repeat(args), itertools.repeat(tmp_dir), batch_dirs),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-fnFile", "--fnFile", dest="fn_file", required=False)
    parser.add_argument("-fnDir", "--fnDir", dest="fn_dir", required=False)
